...
//		ClusterApiService clusterApi = ClusterApiSesrvice.new ClusterApiServiceImpl(siteContexte, vertx);
		// Register the handler
		ServiceBinder clusterApiBinder = new ServiceBinder(vertx);
		clusterApiBinder.setAddress("ClusterApi-address");
		MessageConsumer<JsonObject> clusterApiConsumer = clusterApiBinder.register(ClusterApiServiceImpl.class, clusterApi);
		...
		ClusterApiServiceImpl clusterApiService = clusterApiBuilder.setOptions(new DeliveryOptions(new JsonObject())).build(ClusterApiServiceImpl.class);
		...
		Future<Void> etapesFutures = preparerDonnees(siteContexte).compose(
			a -> configurerCluster(siteContexte).compose(
				b -> configurerOpenApi(siteContexte).compose(
					c -> configurerServices(siteContexte).compose(
						d -> demarrerServeur(siteContexte)
					)
				)
			)
		);
